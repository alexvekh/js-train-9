...
function findElementGreaterThan(arr, num) {
  if (!Array.isArray(arr)) return undefined;
    ...
    if (typeof result === undefined) return "Числа яке задовільняе умову не знайдено";
    else return result;
  }
...
console.log(getSubArrayAndConvertToString(["Я", "люблю", "JavaScript!", "Він", "дуже", "потужний"], 3));
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  if (!Array.isArray(arr)) return "Вхідний параметр має бути масивом";
    ...
    const sum = arr.reduceRight((accum, el) => accum + el);
    arr.forEach((el) => console.log(el));
    return sum;
  }
}
...
function expandArrayByN(arr, n) {
  ...
  if (!Array.isArray(arr) || typeof n !== "number") return "Перший вхідний параметр має бути масивом, другий - числом";
  else {
...
function findDuplicateElements(arr) {
  ...
  if (!Array.isArray(arr)) return " ";
    ...
    const duplicateElements = arr.filter((element, index) => {
      return arr.indexOf(element) !== index && arr.indexOf(element) !== arr.lastIndexOf(element);
    });
...
function capitalizeWords(arr) {
  ...
  if (!Array.isArray(arr)) return " ";
    ...
    const capitalizedArray = arr.map((element) => element.slice(0, 1).toUpperCase() + element.slice(1)); // or   charAt(0).toUpperCase();
...
function calculateTotalPrice(arr) {
  ...
  if (!Array.isArray(arr)) return 0;
    ...
    const totalPrice = arr.reduce((total, element) => (total = total + element.price), 0);
    return totalPrice;
  }
}
